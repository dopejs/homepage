...
export const zh: ProjectCopyMap = {
  ...
  'dsh-tui': {
    tagline: '为 DeepSeek Harness 打造的 Claude Code 风格终端界面。',
    summary:
      'dsh-tui 以 out-of-tree 的 Harness bundle 形式分发，与 Agent 运行时运行在同一进程内。它通过 `ctx.agents` 创建与恢复 Agent，不依赖 Web 客户端代码即可渲染持久化的 session/event 日志，并为审批、提问与命令提供终端适配层。用 npm 安装，用 `dtui` 启动。',
    body: [
      '它是 Harness 插件，而不是独立客户端：与 Agent 运行时同进程运行。同进程架构是刻意的选择 —— 远程传输被留作后续可能的适配器与产品形态，而不是混进第一版实现里。',
      '界面对齐 Claude Code：转录区是一条连贯的对话流，而不是互相争抢屏幕的分栏；助手输出的 Markdown 边流式返回边渲染；Agent 运行时有一行状态报告已用时间与吞吐。推理过程折叠在 Ctrl-E 之后，并且既不进剪贴板，也不进 `--print` 的输出契约 —— 思考过程不应被当成结论。',
      'Harness 依赖声明为 `^0.1.0-rc.6` 且是可选的：Harness 运行时由 `dsh` CLI 提供，没有任何安装器会替插件安装这些包。每次发布都会对着 host 当前的 `latest` 验证全局安装与本地安装，因此上游发布新的 RC 不会让这个包在无人察觉的情况下变得装不上。',
      '终端体验才是这个项目的实质：多行编辑器支持 Unicode 光标移动、选区、撤销/重做、有界历史与 bracketed paste；提交时解析 `@路径` 引用；转录区跟随输出直到导航将其脱离，并提供有界搜索；Ctrl-P 提供模糊命令面板；Ctrl-O 的会话中心仅在 Agent 空闲且编辑器为空时才允许切换。',
    ],
    highlights: [
      'Claude Code 风格的单栏转录：一条连贯的对话流，面板按需唤出，而不是常驻抢占屏幕。',
      '助手 Markdown 边流式返回边渲染，读取器有界且从不抛错 —— 未闭合的代码围栏会被如实报告，而不是被隐藏。',
      '提交时对着工作区解析 `@路径` 引用，每一次拒绝都会说明原因：越出工作区、不可读、二进制、或没有附件存储。',
      '运行状态行给出已用时间与推理强度；当时间窗太短、无法诚实测量时，宁可不给 token 速率，也不猜一个。',
      '推理折叠在 Ctrl-E 之后，并被排除在剪贴板投影和 `--print` 契约之外，确保它永远不会被当作结论去执行。',
      '界面语言支持中文与英文；未显式选择时跟随宿主 locale。',
      '`dtui` 启动器在首次运行时自举 profile，升级后自动对齐版本，而不是打印一条命令让你重敲。',
      ...
      '宿主没有提供的能力会如实报告为「不可用」，而不是编造 —— 任务输出、hooks、MCP 健康状态与成本都会明说。',
    ],
    commandLabels: ['安装', '启动 TUI', '只读环境检查'],
    requirements:
      'Node.js ^22.19.0 || >=24.0.0，以及供应商凭据（如 DEEPSEEK_API_KEY）。`dsh` CLI 把 profile 的安装委托给 pnpm，因此首次运行时机器上需要有 pnpm。',
  },
