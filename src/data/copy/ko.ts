...
export const ko: ProjectCopyMap = {
  ...
  'dsh-tui': {
    tagline: 'DeepSeek Harness를 위한 Claude Code 스타일 터미널 인터페이스.',
    summary:
      'dsh-tui는 out-of-tree Harness 번들로 배포되며 에이전트 런타임과 같은 프로세스에서 실행됩니다. `ctx.agents`를 통해 에이전트를 생성하고 재개하며, 웹 클라이언트 코드에 의존하지 않고 지속되는 session/event 로그를 렌더링하고, 승인·질문·명령을 위한 터미널 어댑터를 제공합니다. npm으로 설치하고 `dtui`로 실행합니다.',
    body: [
      '이것은 별도의 클라이언트가 아니라 Harness 플러그인이며, 에이전트 런타임과 같은 프로세스에 존재합니다. 동일 프로세스 구조는 의도된 선택입니다 —— 원격 전송은 첫 구현에 섞지 않고 이후의 어댑터이자 제품 형태로 남겨 두었습니다.',
      '인터페이스는 Claude Code를 따릅니다. 트랜스크립트는 화면을 다투는 분할 창이 아니라 하나의 이어지는 대화로 읽히고, 어시스턴트의 Markdown은 스트리밍되는 동안 렌더링되며, 실행 중에는 경과 시간과 처리량을 알리는 한 줄이 표시됩니다. 추론은 Ctrl-E 뒤로 접히고 클립보드와 `--print` 출력 계약 어디에도 포함되지 않습니다. 사고 과정이 결론으로 오인되지 않도록 하기 위해서입니다.',
      'Harness peer는 `^0.1.0-rc.6`이자 optional로 선언됩니다. Harness 런타임은 `dsh` CLI가 제공하며, 플러그인을 대신해 그 패키지들을 설치하는 주체가 없기 때문입니다. 릴리스마다 호스트의 현재 `latest`에 대해 전역 설치와 로컬 설치를 모두 검증하므로, 상류의 새 RC가 아무도 모르게 설치 불가를 만들 수 없습니다.',
      '터미널 사용성이 이 프로젝트의 본질입니다. 유니코드 커서 이동·선택·실행 취소/재실행·상한이 있는 기록·bracketed paste를 갖춘 여러 줄 편집기, 제출 시 해석되는 `@경로` 참조, 조작하기 전까지 출력을 따라가며 상한이 있는 검색을 제공하는 트랜스크립트, Ctrl-P의 퍼지 명령 팔레트, 그리고 에이전트가 유휴 상태이고 편집기가 비었을 때만 전환하는 Ctrl-O 세션 센터.',
    ],
    highlights: [
      'Claude Code 스타일의 단일 열 트랜스크립트: 하나의 이어지는 대화이며, 패널은 상주하지 않고 필요할 때 불러냅니다.',
      '어시스턴트 Markdown을 스트리밍 중에 렌더링합니다. 리더는 상한이 있고 예외를 던지지 않으며, 닫히지 않은 코드 펜스는 숨기지 않고 그대로 보고합니다.',
      '제출 시 작업 공간을 기준으로 `@경로` 참조를 해석하고, 거부할 때는 항상 이유를 밝힙니다: 작업 공간 이탈, 읽기 불가, 바이너리, 첨부 저장소 없음.',
      '경과 시간과 추론 강도를 보여 주는 실행 줄. 구간이 너무 짧아 정직하게 측정할 수 없으면 토큰 속도를 추측하지 않고 표시하지 않습니다.',
      '추론은 Ctrl-E 뒤로 접히고 클립보드 투영과 `--print` 계약에서 제외되어, 결론처럼 실행될 수 없습니다.',
      '인터페이스 언어는 영어와 중국어를 지원하며, 선택하지 않으면 호스트 로케일을 따릅니다.',
      '`dtui` 런처는 첫 실행 시 프로필을 부트스트랩하고 업그레이드 후에는 스스로 버전을 맞춥니다. 다시 입력할 명령을 출력하지 않습니다.',
      'Ctrl-P 명령 팔레트가 Harness 명령과 TUI 내비게이션을 합치므로, 조합 키를 보낼 수 없는 터미널에서도 모든 패널에 도달할 수 있습니다.',
      '접근성은 기본 기능입니다: `default`·`high-contrast`·`no-color` 테마, 색이 아니라 의미로 이름 붙인 톤, 괘선을 쓰지 않는 스크린 리더 모드, 모션 감소와 키 재지정을 검증된 하나의 설정 객체에 모았습니다.',
      '호스트가 제공하지 않는 기능은 지어내지 않고 "사용 불가"로 보고합니다 —— 작업 출력, hooks, MCP 상태, 비용 모두 분명히 밝힙니다.',
    ],
    commandLabels: ['설치', 'TUI 실행', '읽기 전용 환경 점검'],
    requirements:
      'Node.js ^22.19.0 || >=24.0.0과 DEEPSEEK_API_KEY 같은 공급자 자격 증명. `dsh` CLI가 프로필 설치를 pnpm에 위임하므로 첫 실행 시 pnpm이 필요합니다.',
  },
