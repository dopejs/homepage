...
export const ko: ProjectCopyMap = {
  ...
  loopforge: {
    tagline: '데스크톱 워크벤치를 갖춘 독립형 로컬 게임 개발 에이전트.',
    summary:
      'Loopforge는 평범한 게임 저장소 안에서 동작합니다. 아이디어를 플레이 가능한 실험으로 바꾸고, 기술적 증거와 사람의 증거를 모아, 유지할지 접을지 다시 만들지를 분명하게 결정하도록 돕습니다. 에이전트가 제어의 중심이고 데스크톱 Workbench가 사용 창구이며, CLI와 Skills는 손으로 다루는 대상이 아니라 내부 실행 수단입니다.',
    body: [
      '프로젝트를 여는 것이 준비의 전부입니다. Workbench가 네이티브 폴더 선택으로 게임 저장소를 받아 해당 프로젝트의 에이전트를 시작하거나 다시 연결하고, 제한된 프로젝트 컨텍스트를 불러옵니다. 이 과정에서 프로바이더 자격 증명, 환경 변수, 액세스 토큰은 노출되지 않습니다.',
      '인터페이스는 에이전트 프로세스가 아니라 프로젝트를 축으로 구성됩니다. 프로젝트 메뉴가 저장소와 프로젝트 단위 화면을 전환하고, 헤더가 프로젝트 정체성과 필요한 동작을 담으며, 작업 영역에는 모드별 도구가 놓이고, 떠 있는 모드 툴바가 탐색·디자인·빌드·테스트 사이를 오갑니다. 채팅은 작업을 대체하지 않고 그 옆에 있습니다. 프로젝트 상태와 증거가 게임 저장소에 남기 때문에, 무슨 일이 있었는지에 대한 기록이 채팅 로그 하나에 달려 있지 않습니다.',
      '경계는 의도적으로 그어져 있습니다. `apps/agent`는 도메인 에이전트와 사용자에게 보이는 동작을, `cli`는 결정론적 프로젝트 작업과 헤드리스 어댑터를 담당하되 제어의 중심은 분명히 아닙니다. `skills`는 판단이 필요한 작업을 위한 버전화된 능력이고, `contracts`는 버전화된 스키마입니다. Kura는 범용 모델·세션·런타임 동작을 제공하며 Loopforge 상태는 전혀 갖지 않고, 릴리스 앱은 에이전트와 고정된 Kura 사이드카를 함께 담습니다.',
    ],
    highlights: [
      '데스크톱 Workbench(Tauri + React)가 제품의 본체이며, 관리하는 사이드카는 에이전트뿐입니다.',
      '프로젝트는 평범한 게임 저장소에서 열리고, 불러오는 컨텍스트에는 자격 증명과 환경 변수가 포함되지 않습니다.',
      '탐색·디자인·빌드·테스트를 오가는 모드 툴바, 그리고 작업을 대체하지 않고 옆에 있는 에이전트 채팅.',
      '상태와 증거가 저장소에 기록되므로 무슨 일이 있었는지가 채팅 로그에 의존하지 않습니다.',
      '결정론적 작업과 버전화된 Skills는 자동화와 진단을 위해 남아 있습니다. 내부 능력이지 진입점이 아닙니다.',
      '범용 모델·세션·런타임은 고정된 Kura 사이드카가 제공하고, 도메인 로직은 Loopforge 쪽에 남습니다.',
    ],
    commandLabels: ['체크아웃에서 Workbench 실행'],
    requirements:
      'Git, Node.js 22와 pnpm, Rust와 Cargo, Python 3.11+와 uv. Godot 4는 Godot 워크플로를 쓸 때만 필요합니다. 아직 패키징된 릴리스가 없어 데스크톱 앱은 체크아웃에서 빌드합니다.',
  },
