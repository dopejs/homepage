...
export const zhTW: ProjectCopyMap = {
  ...
  loopforge: {
    tagline: '獨立的本地遊戲開發 Agent，配桌面工作台。',
    summary:
      'Loopforge 直接在一般的遊戲儲存庫裡工作：把想法變成可玩的實驗，蒐集技術與人工兩類證據，協助你明確做出保留、捨棄或重構的決定。Agent 是控制平面，桌面 Workbench 是使用介面；CLI 與 Skills 是內部能力，不需要你手動驅動。',
    body: [
      '打開專案就是全部的準備工作：Workbench 透過系統資料夾選擇器接入一個遊戲儲存庫，接著自動啟動或重新連線該專案的 Agent，並載入受限的專案脈絡 —— 過程中不暴露供應商憑證、環境變數或存取權杖。',
      '介面是圍繞專案而非圍繞 Agent 行程組織的。專案選單切換儲存庫與專案層級檢視，標題區承載專案識別與必要操作，主工作區放置各模式專屬工具，浮動模式工具列在探索、設計、建置、測試之間切換。聊天位於工作區旁而非取而代之，因為專案狀態與證據都寫在遊戲儲存庫裡 —— 聊天紀錄不是唯一的事實來源。',
      '邊界是刻意劃分的：`apps/agent` 承載領域 Agent 與所有使用者可見行為；`cli` 承載確定性的專案操作與無頭轉接器，明確不是控制平面；`skills` 承載需要情境判斷的版本化能力；`contracts` 承載版本化的結構定義。Kura 提供通用的模型、工作階段與執行時能力，不含任何 Loopforge 狀態；發布版應用內嵌 Agent 與鎖定版本的 Kura sidecar。',
    ],
    highlights: [
      '桌面 Workbench（Tauri + React）是產品本體，只負責 Agent sidecar 的生命週期。',
      '專案直接從一般遊戲儲存庫開啟；載入的專案脈絡不含憑證與環境變數。',
      '模式工具列涵蓋探索、設計、建置、測試，Agent 聊天在工作區旁而非取代工作區。',
      '狀態與證據寫入儲存庫，「發生過什麼」不依賴聊天紀錄。',
      '確定性操作與版本化 Skills 仍可用於自動化與診斷 —— 它們是內部能力，不是入口。',
      '通用的模型、工作階段與執行時來自鎖定版本的 Kura sidecar，領域邏輯留在 Loopforge 這一側。',
    ],
    commandLabels: ['從原始碼執行 Workbench'],
    requirements:
      'Git、Node.js 22 與 pnpm、Rust 與 Cargo、Python 3.11+ 與 uv；僅在使用 Godot 流程時需要 Godot 4。目前沒有打包發布版，桌面應用需從原始碼建置。',
  },
